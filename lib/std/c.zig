...
pub usingnamespace switch (builtin.os.tag) {
    // WASI-libc should have an empty library_name on extern statements for libc functions
    // so that we don't have "wasm-import-{name,module}" on these function symbols. Otherwise,
    // symbols are not resolved by wasm-ld at link time and supposed to be
    // resolved at load time by runtimes. So instead we define them in "c/wasi.zig".
    .wasi => struct {},
    else => struct {
